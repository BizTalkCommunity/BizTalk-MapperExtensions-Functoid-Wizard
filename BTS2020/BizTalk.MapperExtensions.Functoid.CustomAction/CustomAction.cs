    ...
    public class CustomActions : Installer
    {
        ...
        public CustomActions()
        {
            ...
            try
            {
                ...
                regkey = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Microsoft\VisualStudio\Setup");
                if (regkey == null) throw new Exception("Visual Studio doesn't seem to be installed.");
                try
                {
                    this._VisualStudioInstallPath = regkey.GetValue("SharedInstallationPath").ToString();
                }
                catch
                {
                    this._VisualStudioInstallPath = @"C:\Program Files (x86)\Microsoft Visual Studio\Shared";
                }
                regkey.Close();
